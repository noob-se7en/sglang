...
class MlxFxGraphPlan:
    ...
    def require_fully_supported(self) -> None:
        ...
        if not unsupported:
            return
        details = ", ".join(
            f"{node.node_op}:{node.target}" for node in unsupported
        )
...
def _resolve_fx_value(value: Any, values: dict[torch.fx.Node, Any]) -> Any:
    ...
    if isinstance(value, dict):
        return {
            key: _resolve_fx_value(item, values) for key, item in value.items()
        }
    return value
...
def _lower_mlx_node(
    ...
    if lowering == "rms_norm":
        ...
        if weight is None:
            raise UnsupportedMlxFxGraphError(
                "MLX RMSNorm requires an explicit weight"
            )
        if epsilon is None:
    ...
    if lowering == "sdpa":
        ...
        attention_mask = (
            args[3] if len(args) > 3 else kwargs.get("attn_mask")
        )
    ...
    if lowering in {"reshape", "view"}:
        shape = (
            args[1]
            if len(args) == 2 and isinstance(args[1], Sequence)
            else args[1:]
        )
